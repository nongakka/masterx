...
console.log("EP HTML size:", data.length)
  ...
  // หา embed url จาก script
const embedMatch = data.match(/https?:\/\/www\.anime-masterx\.com\/embed\/[0-9]+[^"' ]*/)
...
if(embedMatch){
  ...
  const embedUrl = new URL(embed, epUrl).href
console.log("➡️ open embed:", embedUrl)
  ...
  headers:{
    ...
    "Origin": "https://www.anime-masterx.com"
  }
})
...
const frameMatch =
  html.match(/https?:\/\/stream\.animexd-master\.xyz\/frame\/[a-zA-Z0-9]+/)
...
if(frameMatch){
  ...
  const idMatch = frame.match(/frame\/([a-zA-Z0-9]+)/)
  const id = idMatch ? idMatch[1] : null

  if(id){

    const stream720 =
      `https://stream.animexd-master.xyz/qualitys/${id}/720/`

    const stream360 =
      `https://stream.animexd-master.xyz/qualitys/${id}/360/`
